...
public class PruningBasedMSERForDIBCO extends PruningBasedMSER{
	...
	public boolean isNodeText(NodeCT node){
		...
		double relationWH = Math.max(node.getWidthNode(), node.getHeightNode()) / (double)Math.min(node.getWidthNode(), node.getHeightNode()); 
		if(node.getHeightNode() < tree.getInputImage().getHeight() * 0.55  
				&& 1 <= relationWH && relationWH <= 8 
				&& 15 <= node.getArea()   
				&& 5 <= node.getHeightNode()
				&& node.getWidthNode() < 210
