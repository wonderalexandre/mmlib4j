...
public class ComputerCentralMomentAttributeMergeTree extends AttributeUpdatedIncrementally {
	...
	public void mergeChildrenUpdate(NodeMergedTree node_, NodeMergedTree son_) {	
		double sumLevel2 = son_.getAttributeValue(Attribute.VARIANCE_LEVEL) + (Math.pow(son_.getAttributeValue(Attribute.VOLUME), 2) / son_.info.getAttributeValue(Attribute.AREA));
	...
	public void posProcessing(NodeMergedTree node_) {
		...
		double n = node_.info.getAttributeValue(Attribute.AREA);		
	...
	public class CentralMomentsAttribute {		
		...
		public CentralMomentsAttribute(NodeMergedTree node_){
			...
			this.levelMean = new Attribute(Attribute.LEVEL_MEAN, node_.getAttributeValue(Attribute.VOLUME) / node_.info.getAttributeValue(Attribute.AREA));
		}										
	}
