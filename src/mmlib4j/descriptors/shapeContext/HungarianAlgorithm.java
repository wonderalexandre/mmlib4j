...
public class HungarianAlgorithm {//implements AssignmentAlgorithm{
    ...
    public static int[][] solve(float[][] costMatrix) {
        ...
        float[][] copyMatrix = new float[costMatrix.length][];
        for (int i = 0; i < costMatrix.length; i++) {
        	copyMatrix[i] = new float[costMatrix[i].length];
            System.arraycopy(costMatrix[i], 0, copyMatrix[i], 0, costMatrix[i].length);
        }
        
    	return computeAssignments(copyMatrix);
    }
